...
"""
Author : Yuanqing Mei
File: 2_1_cla_threshold.py
Date : 2022/5/26 11:48
HomePage : https://github.com/meiyuanqing
Email : dg1533019@smail.nju.edu.cn

用每个项目上所有的版本作为测试集时，在用cla方法筛选出类后，在上面应用5种方法计算阈值

cla方法：应用中位数阈值的方法在测试集各版本上生成伪标签的数据。

Based on the data sets obtained from the cla method + median of each OO metric,
...
def cla_threshold_on_test(work_dir, result_dir):
    ...
    for project_t in projects_t:
        ...
        for root, dirs, files in os.walk(work_dir + project):
            ...
            for name in files:
                ...
                if os.path.exists(result_dir + project + '\\cla_threshold_' + name):
                ...
                for metric in metric_data:
                    ...
                    cla_threshold = cla_threshold.append(
                    ...
                    # break
                cla_threshold.to_csv(result_dir + project + '\\cla_threshold_' + name, index=False)
...
if __name__ == '__main__':
    ...
    work_directory = "F:\\unsupervised-threshold-application\\cla\\pseudo_data\\"
    result_directory = "F:\\unsupervised-threshold-application\\cla\\thresholds\\"
    os.chdir(work_directory)

    cla_threshold_on_test(work_directory, result_directory)
